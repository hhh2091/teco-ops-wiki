---
id: doc-teco-ops-docs-p07
title: "Teco-Ops 项目文档集 — 第p07部分 (teco-ops-hardware.md)"
type: source-doc-part
parent_doc: doc-teco-ops-docs
product_version: "commit d90ddf5"
source_file: "doc/teco-ops-hardware.md"
raw_text: "sources/docs/raw-teco-ops/teco-ops-hardware.txt"
raw_line_range: "1-238"
architectures: [sdaa, teco-t1]
confidence: verified
tags: [teco-ops, hardware, spa, spe, spmd, kernel, storage, dma]
---

# Teco-Ops 项目文档集（第p07部分：算子开发硬件相关知识）

> 原文包含多张示意图（`doc/images/异构计算.png`、`AI加速卡.png`、`ai加速卡结构.png`、`kernel函数.png`、`存储模型.png`、`数据传输.png`），本知识库为纯文本知识库，未一并收录，各引用处已保留原图说明文字。
...
太初AI加速卡由多个计算核心阵列SPA组成，每个计算核心阵列SPA由 threadDim 个计算核心SPE和Global存储空间构成。每个计算核心SPE都拥有独立的计算、访存和控制功能，是一个独立的处理器核心，可以独立完成一个计算任务。每个计算核心SPE由FU、SU、SREG、VPU、VREG以及SPM存储空间组成。
...
SDAA C提供了threadIdx和threadDim为每个SPE定制计算任务。其中：
...
- 通过`__global__`关键字标明Kernel函数。
- 在主程序中使用`<<<…>>>`语法调用Kernel函数，将计算任务发送到太初AI加速卡上执行计算。
- 使用`threadIdx`标识执行计算的SPE。
...
- 堆空间：需要手动管理的一块动态内存空间，可以通过malloc接口分配堆内存空间，通过free接口释放所申请的堆内存空间。可通过`get_heap_size`接口查询当前计算核心SPE的堆空间总大小。
...
Kernel函数在设备端运行时所需要的源数据，需要从主机端获取。在进行数据传输前，需要在设备端申请相应大小的Global存储空间，主机端将数据传输到已分配的Global存储空间，完成数据计算后，再将计算结果传回主机端内存，并释放申请的Global存储空间。具体可参考 sdaaMemcpy 接口。

### 同一计算核心阵列SPA的Global存储空间内的数据传递

同一计算核心阵列SPA内的Global存储空间和Global存储空间之间的数据传递，可参考 memcpy 接口。

### 不同计算核心SPE之间的数据传递

为使不同计算核心SPE之间的数据交换更加方便，SDAA C提供了RMA编程接口，用于搬运不同SPM存储空间之间的数据。RMA编程接口实现了计算核心SPE之间的高效数据搬运，且可以缓解SPM存储空间容量受限的问题。

### 在同一计算核心SPE的SPM存储空间内的数据传递
计算核心SPE可在其内部的SPM空间进行数据传递，可参考 memcpy 接口。
