---
id: doc-teco-al-docs-p01
title: "Teco-AL 项目文档集 — 第p01部分 (README.md)"
type: source-doc-part
parent_doc: doc-teco-al-docs
product_version: "v1.0.0 / commit 17c5cd6"
source_file: "README.md"
raw_text: "sources/docs/raw-teco-al/README.txt"
raw_line_range: "1-416"
architectures: [sdaa, teco-t1]
confidence: verified
tags: [teco-al, readme, directory-structure, interface, ual, pr-convention, tecotest]
---

# Teco-AL 项目文档集（第p01部分：README.md）
...
- SDAA C编程指南：介绍SDAA C编程语言、语言规范、函数接口、数学函数、程序编译、程序调试及性能调优等内容。
- 性能优化手册-SDAAC篇：介绍程序并行、函数接口、数学函数、程序编译过程中的性能优化内容。
- 性能优化手册-算子篇：介绍经典的计算与访存优化办法。前者含向量指令、指令流水线、矩阵乘法加速单元；后者含双缓冲、广播优化办法。

Teco-AL仓库中的常用概念解释，例如：ops、ual、algo、args等缩略语；句柄、张量、存储格式、描述符、工作空间等通用概念，可以按需查阅《开发指南——附录：核心概念》（`doc-teco-al-docs-p02`）了解详情。

## 算子支持现状

Teco-AL仓库中每个实现的算子，均有一对一配套的算子设计文档，用户可以查阅`./doc/op_docs/`目录，了解Teco-AL仓库目前已实现的算子，以及各个算子的功能特性与性能优化方案。各个算子文档包含3大内容模块：
...
- 实现示例：例如`interface/include/tecoal.h`中的`tecoalAddTensor`属于userAPI；该接口中使用的`tecoalAlgo_t`属于自定义枚举；该接口中使用的`tecoalTensorDescriptor_t`属于自定义结构体。

##### ops层（算子定义与实现）

- 路径地址：`interface/ops/{opname}.cpp`
- 代码介绍：ops层接口实现代码，含：参数赋值、功能函数调用等功能。功能函数调用，通过`RUN_OP`实现，传入参数均由`args`封装，具有更好的扩展性。
- 实现示例：详见`interface/ops/add_tensor.cpp`中的代码及其注释。
...
- 实现示例：详见`ual/args/add_tensor_args.h`中的代码及其注释。
...
- 代码介绍：
    ...
    - `.cpp`文件包含具体分支进入的判断逻辑。
    - `.h`文件为对应头文件。
- 实现示例：详见`ual/ops/add_tensor/`目录下的代码及其注释。
...
- 代码介绍：
    ...
    - `.h`文件为对应头文件。
- 实现示例：详见`ual/kernel/add_tensor/`目录下的代码及其注释。

## 贡献指南

### 步骤一：fork仓库

将Teco-AL官方仓库fork到贡献者的个人空间，点击仓库页面右上方的fork按钮即可。
...
- "分支派发"：定义了不同计算分支的进入判断条件。
- "计算实现"：对应不同计算实现的底层逻辑代码。
...
此外，贡献者需要注意：
- SPM内存申请时，不能超过235KB，推荐使用仓库封装的`rt_spm_malloc()`与`rt_spm_free()`等接口（`ual/com/rt.h`，接口封装占用128B，上限为235KB-128B=240512B），可以在超量申请时，报错提醒。
- 新增文件需要参考仓库已有文件，统一在文件头添加BSD License。
- 编码整体使用《Google C++ 风格》，贡献者可以使用`format2google`脚本将代码规范化。再根据pre-commit自动触发的cpplint检查结果，按需规范化代码。
- 将自己新增的特性，更新到对应算子的设计文档中（`./doc/op_docs/`目录下），保证代码与文档信息无偏差，便于其他开发者查阅。

更多指引说明，可查阅《开发指南》（`doc-teco-al-docs-p02`）。
...
出现其余结果，则表示运行失败。例如若出现下方提示，用户需要添加正确的算法分支指定数字，具体范围需要查阅对应算子设计文档中`algo`参数的介绍，如《tecoalAddTensor设计文档》（`doc-teco-al-docs-p05`）写明`algo`范围是0~5的整数。
...
- SPM空间申请时，不要超过235KB。推荐使用仓库封装的`rt_spm_malloc()`与`rt_spm_free()`等接口（`ual/com/rt.h`，上限240512B），可以在超量申请时，报错提醒。
- 针对自己开发的分支，需要完善`./doc/op_docs/`目录下的算子设计文档，说明性能优化思路，如数据分块、流水设计、广播操作等
- `git commit`提交的message信息，需要符合commit_template模板规范，即`[<question_num>](<algo_num>): <subject>`格式，否则可能会影响CI环境上的测试结果
...
> 原文此处附有一张截图（`doc/tutorial/pics/private.png`），演示如何将个人 fork 仓库设为私有，本知识库未收录图片。
...
Teco-AL采用The 3-Clause BSD License。具体内容，请参见仓库根目录`LICENSE`文件；版权声明详见`NOTICE`文件（太初（无锡）电子科技有限公司，© 2024 TECORIGIN Co., Ltd. All rights reserved.）。
